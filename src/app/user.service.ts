...
//get method: https://stackoverflow.com/questions/34475523/how-to-pass-url-arguments-query-string-to-a-http-request-on-angular
  ...
  deleteUser (user: User | number): Observable<User> {
    const id = typeof user === 'number' ? user : user.personID;
    //const url = `${this.urlDelete}/${id}`;

    return this.http.delete<User>(this.urlDelete, {params: {personID: id.toString()}
    }).pipe(
  ...
  updateUser (user: User): Observable<any> {
    ...
    return this.http.put<User>(this.urlUpdate, user, httpOptions).pipe(
      tap(_ => this.log(`updated user id=${user.personID} name=${user.username}`)),
